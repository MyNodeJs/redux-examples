...
export const REQUEST_TOPIC_PUBLISH_POSTS = 'REQUEST_TOPIC_PUBLISH'
export const RECEIVE_TOPIC_PUBLISH_POSTS = 'RECEIVE_TOPIC_PUBLISH'
...
export function fetchMyMessagesGet(accesstoken, mdrender = true) {
  return dispatch => {
    ...
    return fetch(
      `http://localhost:8081/api/v1/messages?accesstoken=${accesstoken}&mdrender=${mdrender}`
    )
      .then(response => response.json())
...
export function fetchTopicsGet(
  page = 1,
  tab = "",
  limit = 100,
  mdrender = true
  ...
  return dispatch => {
    ...
    return fetch(
      `http://localhost:8081/api/v1/topics?page=${page}&tab=${tab}&limit=${limit}&mdrender=${mdrender}`
    )
      .then(response => response.json())
...
export function requestTopicPublishPosts() {
  return {
    type: REQUEST_TOPIC_PUBLISH_POSTS
  }
}

export function receiveTopicPublishPosts(json) {
  return {
    type: RECEIVE_TOPIC_PUBLISH_POSTS,
    posts: json
  }
}

export function fetchTopicPublishPosts(accesstoken, title, tab, content) {
  return dispatch => {
    dispatch(requestTopicPublishPosts());
    return fetch('http://localhost:8081/api/v1/topics', {
      method: "POST",
      headers: {
        "Content-Type": "application/x-www-form-urlencoded"
      },
      ...
      .then(response => response.json())
      .then(json => dispatch(receiveTopicPublishPosts(json)));
  };
}